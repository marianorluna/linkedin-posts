...
- **Dominio / casos de uso:** `lib/domain/` (`brand-kit-service`, posts)
- **Infra (Prisma, LLM, export):** `lib/infra/`
- **Studio UI:** `app/page.tsx`, `app/posts/*`, `components/studio/` (StylePanel, IconPicker)
...
4. Guardar versión (`Post` + `PostVersion`).
5. Export PDF: `/api/export` (Playwright + pdf-lib). Requiere `pnpm dev` en `NEXT_PUBLIC_APP_URL`.
6. Subir el PDF a LinkedIn como documento.
